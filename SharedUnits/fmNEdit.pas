...
type
  TNedit = class(TForm)
    quInsertNote: TADOQuery;
    quUpdateNote: TADOQuery;
    Panel1: TPanel;
    Button2: TButton;
    Button1: TButton;
    Panel2: TPanel;
...
begin
  if mde=1 then
   begin
     quInsertnote.Parameters[0].value:=salg;
     quInsertnote.Parameters[1].value:=mahg;
     quInsertnote.Parameters[2].value:=rouzg;
     quInsertnote.Parameters[3].value:=CdPer;
     quInsertnote.Parameters[4].assign(memo1.lines);
     quInsertnote.ExecSql;
     if cbseda.Checked then putsOnper;
   end
  else
   begin
     quUpdateNote.Parameters[0].assign(memo1.lines);
     quUpdateNote.Parameters[1].value:=salg;
     quUpdateNote.Parameters[2].value:=mahg;
     quUpdateNote.Parameters[3].value:=rouzg;
     quUpdateNote.Parameters[4].value:=CdPer;
