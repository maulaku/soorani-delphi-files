...
uses
  ...
  ExtCtrls, StdCtrls, DBCtrls, DBGrids, Buttons, Spin, Grids,define,db,RadForms,
  ComCtrls, DBTables, Menus, OleCtnrs,variants, ADODB,Fard;

type
  TNote = class(TRadForm)
    quDelSch: TADOQuery;
    quGetTatil: TADOQuery;
    quCntTab: TADOQuery;
...
begin
     if (sgTst.cells[sgTst.col,sgTst.Row]<>'')  then
       begin
        quChkNewNote.Close;
        quChkNewNote.Parameters[0].value:=DabData.emp_no;
        quChkNewNote.Parameters[1].Value:=strtoint(copy(inttostr(seSal.value), 3, 2));
        quChkNewNote.Parameters[2].Value:=Fdate.MahAdad(cbMah.text);
        quChkNewNote.Parameters[3].Value:=strtoint(sgTst.Cells[sgTst.Col,sgTst.Row]);
        quChkNewNote.Open;
        if quChkNewNote.RecordCount=0 then
         begin
            Nedit:=Tnedit.Create(Application);
            Nedit.salg:=strtoint(copy(inttostr(seSal.value), 3, 2));
            Nedit.Mahg:=Fdate.MahAdad(cbMah.text);
            Nedit.Rouzg:=strtoint(sgTst.Cells[sgTst.Col,sgTst.Row]);
            Nedit.CdPer:=DabData.emp_no;
            Nedit.mde:=1;
            Nedit.Showmodal;
            Nedit.free;
            SetMeetPos;
         ...
         ShowAlertMessage('œ— «Ì‰ —Ê“ ﬁ»·« Ì«œœ«‘  ê–«‘ Â ‘œÂ «” ',1);
       end;
end;
...
begin
  if (sgTst.cells[sgTst.col,sgTst.Row]<>'') and (quCntTabSal.asstring<>'')  then
    begin
      ...
      MyBook:=quCntTab.GetBookMark;
      quCntTab.Close;
      quCntTab.Parameters[0].value:=DabData.emp_no;
      quCntTab.Parameters[1].Value:=strtoint(copy(inttostr(seSal.value),3,2));
      quCntTab.Parameters[2].Value:=Fdate.MahAdad(cbMah.text);
      quCntTab.Parameters[3].Value:=strtoint(sgTst.Cells[sgTst.Col,sgTst.Row]);
      quCntTab.Open;
      RefNote(sgTst.Col,sgTst.Row);
      if not quCntTabSole.IsNull then taTabNote.RefResh;
      quCntTab.GotoBookMark(MyBook);
...
begin
  if quCntTab.fieldbyname('DefCd').asstring<>'' then
    begin
     quDelSch.Parameters[0].Value:=quCntTabSal.Value;
     quDelSch.Parameters[1].Value:=quCntTabMah.Value;
     quDelSch.Parameters[2].Value:=quCntTabRouz.Value;
     quDelSch.Parameters[3].value:=DabData.emp_no;
     quDelSch.ExecSql;
     RefNote(sgTst.col,sgTst.row);
     SetMeetPos;
     sgTst.Refresh ;
     quCntTab.Close;
     quCntTab.Parameters[0].value:=CdPer;
     quCntTab.Parameters[1].Value:=strtoint(copy(inttostr(seSal.value),3,2));
     quCntTab.Parameters[2].Value:=Fdate.MahAdad(cbMah.text);
     quCntTab.Parameters[3].Value:=strtoint(sgTst.Cells[sgTst.Col,sgTst.Row]);
     quCntTab.Open;
...
begin
  ...
  quCntTab.Close;
  quCntTab.Parameters[0].value:=CdPer;
  quCntTab.Parameters[1].Value:=p1;
  quCntTab.Parameters[2].Value:=p2;
  quCntTab.Parameters[3].Value:=p3;
  quCntTab.Open;
...
procedure TNote.SetNow;
 var i,j   :integer;
    m1,y1 :integer;
 begin
  y1:=1300+strtoint(copy(Dabdata.disp_date,1,2));
  seSal.value:=y1;
  m1:=strtoint(copy(Dabdata.disp_date,7,2));
  cbMah.itemindex:=strtoint(copy(Dabdata.disp_date,4,2))-1;
  cbMahChange(nil);
  for i:=0 to 6 do
    for j:=1 to 6 do
     begin
...
begin
  kc:=1;
  quGetTatil.Close;
  quGetTatil.Parameters[0].Value:=strtoint(copy(inttostr(seSal.value),3,2));
  quGetTatil.Parameters[1].Value:=Fdate.MahAdad(cbMah.text);
  quGetTatil.Open;
  quGetTatil.First;
  mmTat.Clear ;
...
begin
  Kcnt:=1;
  quGetTr.Close;
  quGetTr.Parameters[0].value:=CdPer;
  quGetTr.Parameters[1].Value:=strtoint(copy(inttostr(seSal.value),3,2));
  quGetTr.Parameters[2].Value:=Fdate.MahAdad(cbMah.text);
  quGetTr.Open;
  quGetTr.First;
