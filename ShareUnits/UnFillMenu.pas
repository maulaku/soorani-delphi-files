...
begin
  ...
  OutLines1.Add(TrimDash(StringReplace(StartTemp,'$Title','ÇãæÇá',[rfReplaceAll, rfIgnoreCase])));
  for i:=0 to Menu.Items.Count-1 do
   begin
    ...
    for k:=0 to Menu.Items[i].Count-1 do
     begin
        ...
        begin
           ...
           OutLines4.SaveToFile('D:\test\'+Menu.Items[i].Items[k].Items[l].Name+'.htm');
           OutLines4.Clear;
        end;
        OutLines3.Add(EndTemp);
        OutLines3.SaveToFile('D:\test\'+Menu.Items[i].Items[k].Name+'.htm');
        OutLines3.Clear;
        OutLines2.Add('</ul>'+EOF);
     end;
     OutLines2.Add(EndTemp);
     OutLines2.SaveToFile('D:\test\'+Menu.Items[i].Name+'.htm');
     OutLines2.Clear;
     OutLines1.Add('</ul>'+EOF);
   end;
   OutLines1.Add(EndTemp);
   OutLines1.SaveToFile('D:\test\Test.htm');
end;
