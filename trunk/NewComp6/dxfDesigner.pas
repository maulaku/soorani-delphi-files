...
//    PreApplicationMessage:TMessageEvent;
...
begin
  ...
  begin
    ...
    Items[0].Caption := 'ÃœÊ· çÌ‰';
    Items[0].Tag := 1;
    Items[0].OnClick := PopupMenuClick;
    Items.Add(TMenuItem.Create(FPopupMenu));
    Items[1].Caption := '›—” «œ‰ »Â Ã·Ê';
    Items[1].Tag := 2;
    Items[1].OnClick := PopupMenuClick;
    Items.Add(TMenuItem.Create(FPopupMenu));
    Items[2].Caption := '›—” «œ‰ »Â ⁄ﬁ»';
    Items[2].Tag := 3;
...
begin
  ...
  if dxfDsCount.FList.Count = 0 then
    Application.OnMessage := nil;//PreApplicationMessage;
end;
...
begin
  ...
  begin
//    if Assigned(PreApplicationMessage) then
//       PreApplicationMessage(Msg,Handled);
    Exit;
  end;
  with Msg do
    case Message of
