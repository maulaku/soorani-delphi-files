...
procedure TreSrtBooks.DoRun(var mes :TMessage);
begin
     i := 1;
...
//     paState.Visible :=true;
     spreStrBook.close;
     spreStrBook.Parameters[1].Value:=meFrom.Text;
     spreStrBook.Parameters[2].Value:=meTo.Text;
     spreStrBook.open;
...
procedure TreSrtBooks.DoPrint(var mes :TMessage);
begin
  ppViewer1.Print;
end;

procedure TreSrtBooks.FormClose(Sender: TObject; var Action: TCloseAction);
begin
 FormDeactivate(nil);
{ recredit:=nil;}
 Action:=CaFree;
...
procedure TreSrtBooks.FormActivate(Sender: TObject);
var mes:TMessage;
begin
   mes.Msg := FORM_STATE;
   mes.Lparam := 124;
...
procedure TreSrtBooks.FormDeactivate(Sender: TObject);
var mes:TMessage;
begin
 mes.Msg:=FORM_DEACT;
 Application.MainForm.Dispatch(mes);
...
procedure TreSrtBooks.spbPreview100PercentClick(Sender: TObject);
begin
  ppViewer1.ZoomSetting := zs100Percent;
...
procedure TreSrtBooks.spbPreviewWidthClick(Sender: TObject);
begin
...
procedure TreSrtBooks.spbPreviewWholeClick(Sender: TObject);
begin
...
procedure TreSrtBooks.mskPreviewPageKeyPress(Sender: TObject; var Key: Char);
var
  liPage: Longint;
begin
...
procedure TreSrtBooks.spbPreviewNextClick(Sender: TObject);
begin
  ppViewer1.NextPage;
  mskPreviewPage.text:=inttostr(ppViewer1.AbsolutePageNo);
end;

procedure TreSrtBooks.spbPreviewPriorClick(Sender: TObject);
begin
  ppViewer1.PriorPage;
  mskPreviewPage.text:=inttostr(ppViewer1.AbsolutePageNo);
end;

procedure TreSrtBooks.mskPreviewPercentageKeyPress(Sender: TObject;
  var Key: Char);
var
...
procedure TreSrtBooks.quFunFormalRowGetText(Sender: TField;
...
procedure TreSrtBooks.ppDBText2GetText(Sender: TObject; var Text: String);
begin
  Text := inttostr(i div 3);
  i := i+1;
end;

procedure TreSrtBooks.FormShow(Sender: TObject);
begin
