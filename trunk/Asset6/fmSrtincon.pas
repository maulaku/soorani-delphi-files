...
procedure TreSrtIncon.DoRun(var mes :TMessage);
begin
     i := 1;
...
//     paState.Visible :=true;
     ...
     if(meTo.Text='  /  /  ') then
       meTo.Text:='99/12/29';
     spreStrIcon.close;
     spreStrIcon.Parameters[1].Value:=meFrom.Text;
     spreStrIcon.Parameters[2].Value:=meTo.Text;
     spreStrIcon.open;
...
procedure TreSrtIncon.DoPrint(var mes :TMessage);
begin
  ppViewer1.Print;
end;

procedure TreSrtIncon.FormClose(Sender: TObject; var Action: TCloseAction);
begin
 FormDeactivate(nil);
{ recredit:=nil;}
 Action:=CaFree;
...
procedure TreSrtIncon.FormActivate(Sender: TObject);
var mes:TMessage;
begin
   mes.Msg := FORM_STATE;
   mes.Lparam := 136;
...
procedure TreSrtIncon.FormDeactivate(Sender: TObject);
var mes:TMessage;
begin
 mes.Msg:=FORM_DEACT;
 Application.MainForm.Dispatch(mes);
...
procedure TreSrtIncon.spbPreview100PercentClick(Sender: TObject);
begin
  ppViewer1.ZoomSetting := zs100Percent;
...
procedure TreSrtIncon.spbPreviewWidthClick(Sender: TObject);
begin
...
procedure TreSrtIncon.spbPreviewWholeClick(Sender: TObject);
begin
...
procedure TreSrtIncon.mskPreviewPageKeyPress(Sender: TObject; var Key: Char);
var
  liPage: Longint;
begin
...
procedure TreSrtIncon.spbPreviewNextClick(Sender: TObject);
begin
  ppViewer1.NextPage;
  mskPreviewPage.text:=inttostr(ppViewer1.AbsolutePageNo);
end;

procedure TreSrtIncon.spbPreviewPriorClick(Sender: TObject);
begin
  ppViewer1.PriorPage;
  mskPreviewPage.text:=inttostr(ppViewer1.AbsolutePageNo);
end;

procedure TreSrtIncon.mskPreviewPercentageKeyPress(Sender: TObject;
  var Key: Char);
var
...
procedure TreSrtIncon.quFunFormalRowGetText(Sender: TField;
...
procedure TreSrtIncon.ppDBText2GetText(Sender: TObject; var Text: String);
begin
  Text := inttostr(i div 3);
  i := i+1;
end;

procedure TreSrtIncon.FormShow(Sender: TObject);
begin
