...
type
  TreSpecRide = class(TRadForm)
    ...
    ppShape1: TppShape;
    quSpecRideFLName: TStringField;
    quSpecRideItem_name: TStringField;
    quSpecRideRec_no: TIntegerField;
    quSpecRideDate: TStringField;
    quSpecRidebPrice: TFloatField;
    quSpecRideEPrice: TFloatField;
    quSpecRideTra_gcode: TStringField;
    quSpecRideTra_type: TSmallintField;
    quSpecRideTra_mcou: TStringField;
    quSpecRideTra_mfac: TStringField;
    quSpecRideTra_eno: TStringField;
    quSpecRideTra_cno: TStringField;
    quSpecRideTra_pno: TStringField;
    quSpecRideTra_color: TStringField;
    quSpecRideTra_mod: TStringField;
    quSpecRideTra_cap: TStringField;
    quSpecRideTra_mdate: TStringField;
    quSpecRideTra_desc: TStringField;
    quSpecRideMyGoodCode: TStringField;
    quSpecRideEmp: TStringField;
    quSpecRidegoods_codeStr: TStringField;
    quSpecRideTypeName: TStringField;
    quSpecRideState_Name: TStringField;
...
procedure TreSpecRide.DoRun(var mes :TMessage);
begin
/// if ((dblUnit.text<>'') and (dblCostCenter.text<>'') and (dblLocCode.text<>'') ) then
   begin
     ...
     quSpecRide.close;
     quSpecRide.Parameters[0].value := OrganizCode;
     if trim(dblunit.text)<>'' then
           quSpecRide.Parameters[1].value := dblUnit.KeyValue
     else quSpecRide.Parameters[1].value :=0;
     if trim(dblCostCenter.text)<>'' then
       quSpecRide.Parameters[2].value := dblCostCenter.KeyValue
     else quSpecRide.Parameters[2].value :=0;
     if trim(dblLocCode.text)<>'' then
        quSpecRide.Parameters[3].value := dblLocCode.KeyValue
     else quSpecRide.Parameters[3].value :=0;
     if trim(dblemp.text)<>'' then
        quSpecRide.Parameters[4].value := dblemp.KeyValue
     else quSpecRide.Parameters[4].value :=0;
...
procedure TreSpecRide.DoPrint(var mes :TMessage);
begin
  ppViewer1.Print;
end;

procedure TreSpecRide.FormClose(Sender: TObject; var Action: TCloseAction);
begin
 FormDeactivate(nil);
{ recredit:=nil;}
 Action:=CaFree;
...
procedure TreSpecRide.FormActivate(Sender: TObject);
var mes:TMessage;
begin
   mes.Msg := FORM_STATE;
   mes.Lparam := 63;
...
procedure TreSpecRide.FormDeactivate(Sender: TObject);
var mes:TMessage;
begin
 mes.Msg:=FORM_DEACT;
 Application.MainForm.Dispatch(mes);
...
procedure TreSpecRide.spbPreview100PercentClick(Sender: TObject);
begin
...
procedure TreSpecRide.spbPreviewWidthClick(Sender: TObject);
begin
...
procedure TreSpecRide.spbPreviewWholeClick(Sender: TObject);
begin
...
procedure TreSpecRide.mskPreviewPageKeyPress(Sender: TObject; var Key: Char);
var
  liPage: Longint;
begin
...
procedure TreSpecRide.spbPreviewNextClick(Sender: TObject);
begin
  ppViewer1.NextPage;
  mskPreviewPage.text:=inttostr(ppViewer1.AbsolutePageNo);
end;

procedure TreSpecRide.spbPreviewPriorClick(Sender: TObject);
begin
  ppViewer1.PriorPage;
  mskPreviewPage.text:=inttostr(ppViewer1.AbsolutePageNo);
end;

procedure TreSpecRide.mskPreviewPercentageKeyPress(Sender: TObject;
  var Key: Char);
var
...
procedure TreSpecRide.quFunFormalRowGetText(Sender: TField;
...
procedure TreSpecRide.ppDBText2GetText(Sender: TObject; var Text: String);
begin
  Text := inttostr(quspecride.RecNo);
end;

procedure TreSpecRide.FormShow(Sender: TObject);
begin
...
procedure TreSpecRide.dblUnitCloseUp(Sender: TObject);
begin
...
procedure TreSpecRide.dblCostCenterCloseUp(Sender: TObject);
begin
      DabData.quDeployLoc.Close;
...
procedure TreSpecRide.dblCostCenterEnter(Sender: TObject);
begin
...
procedure TreSpecRide.dblLocCodeEnter(Sender: TObject);
begin
