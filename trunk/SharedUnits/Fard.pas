...
type
   ...
   public
       ...
       Function disp_date(ADOC:TADOConnection):string;
       Function disp_date_SP(ADOC:TADOConnection):string;
...
var Tmah :string;
    mah1:integer;
 begin
   mah1:=strtoint(copy(disp_date_Local,4,2));
  case mah1 of
   1:tmah:='ÝÑæÑÏíä';
   2:tmah:='ÇÑÏíÈåÔÊ';
   3:tmah:='ÎÑÏÇÏ';
...
function Tfar_date.disp_date(ADOC:TADOConnection): String;
var Qr:TADOQuery;
begin
  if ADOC<>nil then
  ...
  disp_date:=disp_date_Local;
end;

function Tfar_date.disp_date_SP(ADOC:TADOConnection): String;
var Qr:TADOQuery;
begin
  if ADOC<>nil then
   if ADOC.Connected then
     begin
      ...
      Qr.SQL.Text:='SELECT GETDATE() GeregorianDate';
      Qr.Open;
      disp_date_SP:=G2PDate(Qr.Fields[0].AsDateTime);
      exit;
    end;
  disp_date_SP:=disp_date_Local;
end;
