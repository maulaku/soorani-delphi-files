...
uses
  ...
  Menus, Messages, Commctrl, ExtCtrls, StdCtrls, Buttons,grids,ControlTest,
  DbGrids;
...
procedure Register;
begin
  RegisterComponents('XP', [TXPText]);
end;

{ TXPTexte }

constructor TXPText.Create(AOwner: TComponent);
begin
...
procedure TXPText.ActivateMenuItem(MenuItem: TMenuItem);

  procedure Activate(MenuItem: TMenuItem);
  begin
    if (MenuItem.Tag <> 999) then
    if addr(MenuItem.OnDrawItem) <> addr(TXPText.DrawItem) then
    begin
...
procedure TXPText.InitItems(wForm: TWinControl; Enable, Update: boolean );
var
  i: integer;
  Comp: TComponent;
begin
...
procedure TXPText.InitComponent(Comp: TComponent); // Tom: for external (by the main program) use without parameters. "Thomas Knoblauch" <thomas@tom-the-bomb.de> 27.08
begin
  if FActive then InitItem(Comp, true, true);
end;

// Tom: "Thomas Knoblauch" <thomas@tom-the-bomb.de> 27.08
procedure TXPText.InitItem(Comp: TComponent; Enable, Update: boolean );
  ...
  begin
    ...
    begin
      ...
      if addr(MenuItem.OnMeasureItem) = addr(TXPText.MeasureItem) then
    ...
    begin
      ...
      begin
        if addr(TToolBar(Comp).OnCustomDrawButton) =
          addr(TXPText.ToolBarDrawButton) then
    ...
    begin
      ...
      begin
        if addr(TControlBar(Comp).OnBandPaint) =
          addr(TXPText.ControlBarPaint) then
...
procedure TXPText.DrawItem(Sender: TObject; ACanvas: TCanvas; ARect: TRect;
  Selected: Boolean);
begin
  if FActive then
...
function TXPText.GetImageExtent(MenuItem: TMenuItem): TPoint;
var
...
procedure TXPText.MeasureItem(Sender: TObject; ACanvas: TCanvas;
  var Width, Height: Integer);
var
  s: string;
  W, H: integer;
...
procedure TXPText.MenueDrawItem(Sender: TObject; ACanvas: TCanvas; ARect: TRect;
  Selected: Boolean);
var
  txt: string;
  B: TBitmap;
...
{$IFDEF VER5U}
procedure TXPText.ToolBarDrawButton(Sender: TToolBar;
  Button: TToolButton; State: TCustomDrawState; var DefaultDraw: Boolean);
...
procedure TXPText.ControlBarPaint(Sender: TObject; Control: TControl;
  Canvas: TCanvas; var ARect: TRect; var Options: TBandPaintOptions);
var
  i: Integer;
  intInc: integer;
...
procedure TXPText.SetGlobalColor(ACanvas: TCanvas);
begin
//-----
...
procedure TXPText.DrawTopMenuItem(Sender: TObject; ACanvas: TCanvas;
...
procedure TXPText.DrawCheckedItem(FMenuItem: TMenuItem; Selected, Enabled,
 HasImgLstBitmap: boolean; ACanvas: TCanvas; CheckedRect: TRect);
var
  X1, X2: integer;
begin
...
procedure TXPText.DrawTheText(Sender: TObject; txt, ShortCuttext: string;
    ACanvas: TCanvas; TextRect: TRect;
    Selected, Enabled, Default, TopMenu, IsRightToLeft: boolean;
    var TxtFont: TFont; TextFormat: integer);
var
...
procedure TXPText.DrawIcon(Sender: TObject; ACanvas: TCanvas; B: TBitmap;
 IconRect: Trect; Hot, Selected, Enabled, Checked, FTopMenu,
 IsRightToLeft: boolean);
var
  DefColor: TColor;
...
function TXPText.TopMenuFontColor(ACanvas: TCanvas; Color: TColor): TColor;
var
  r, g, b, avg: integer;
begin
...
procedure TXPText.SetActive(const Value: boolean);
begin
...
procedure TXPText.SetAutoDetect(const Value: boolean);
begin
  FAutoDetect := Value;
end;

procedure TXPText.SetForm(const Value: TScrollingWinControl);
var
  Hold: boolean;
begin
  if Value <> FForm then
...
procedure TXPText.SetFont(const Value: TFont);
begin
  FFont.Assign(Value);
  Windows.DrawMenuBar(FForm.Handle);
...
procedure TXPText.SetColor(const Value: TColor);
begin
  FColor := Value;
end;

procedure TXPText.SetIconBackColor(const Value: TColor);
begin
  FIconBackColor := Value;
end;

procedure TXPText.SetMenuBarColor(const Value: TColor);
begin
  FMenuBarColor := Value;
  Windows.DrawMenuBar(FForm.Handle);
end;

procedure TXPText.SetCheckedColor(const Value: TColor);
begin
  FCheckedColor := Value;
end;

procedure TXPText.SetSeparatorColor(const Value: TColor);
begin
  FSeparatorColor := Value;
end;

procedure TXPText.SetSelectBorderColor(const Value: TColor);
begin
  FSelectBorderColor := Value;
end;

procedure TXPText.SetSelectColor(const Value: TColor);
begin
  FSelectColor := Value;
end;

procedure TXPText.SetDisabledColor(const Value: TColor);
begin
  FDisabledColor := Value;
end;

procedure TXPText.SetSelectFontColor(const Value: TColor);
begin
  FSelectFontColor := Value;
end;

procedure TXPText.SetIconWidth(const Value: integer);
begin
  FIconWidth := Value;
end;

procedure TXPText.SetDrawSelect(const Value: boolean);
begin
  FDrawSelect := Value;
end;
...
procedure TXPText.SetOverrideOwnerDraw(const Value: boolean);
begin
...
procedure TXPText.SetUseSystemColors(const Value: boolean);
begin
  FUseSystemColors := Value;
  Windows.DrawMenuBar(FForm.Handle);
end;

procedure TXPText.SetGradient(const Value: boolean);
begin
  FGradient := Value;
end;

procedure TXPText.SetFlatMenu(const Value: boolean);
begin
  FFlatMenu := Value;
end;

procedure TXPText.SetXPContainers(const Value: TXPContainers);
begin
...
procedure TXPText.SetXPControls(const Value: TXPControls);
begin
...
procedure TXPText.SetDrawMenuBar(const Value: boolean);
begin
  FDrawMenuBar := Value;
end;

procedure TXPText.SetUseDimColor(const Value: boolean);
begin
  FUseDimColor := Value;
end;
...
procedure TXPText.DrawGradient(ACanvas: TCanvas; ARect: TRect;
 IsRightToLeft: boolean);
var
  i: integer;
  v: integer;
...
procedure TXPText.DrawWindowBorder(hWnd: HWND; IsRightToLeft: boolean);
var
  WRect: TRect;
  dCanvas: TCanvas;
begin
...
procedure TXPText.Notification(AComponent: TComponent;
...
begin
  ...
  if (Control.ClassName = 'TScrollBar')then
   PaintScroll;
...
procedure TControlSubClass.PaintScroll;
var
  C: TControlCanvas;
  R: TRect;
  M: TRect;
...
begin
  ...
  try
    ...
    if (TScrollBar(Control).Kind=sbHorizontal)  then
    begin
       M.Left:=Trunc(TScrollBar(Control).Position/(TScrollBar(Control).Max)*(Control.Width-30));
       M.right:=M.left+20;
       M.Left:=M.left;
       M.Top:=0;
       M.Bottom:=Control.Height;
    end;
    C.Brush.Color:= SelectColor ;
    C.FillRect(M);
...
//    C.FrameRect(L);
    ...
    L.Left := L.Right - GetSystemMetrics(SM_CXHTHUMB) - 1;
      ...
      C.Moveto(L.Right, L.Top);
      C.LineTo(L.Right, L.Bottom);
      ...
      C.Moveto(X + 0, R.Top + 4);
      C.LineTo(X + 0, R.Top + 11);
      C.Moveto(X + 1, R.Top + 5);
      C.LineTo(X + 1, R.Top + 10);
      C.Moveto(X + 2, R.Top + 6);
      C.LineTo(X + 2, R.Top + 9);
      C.Moveto(X + 3, R.Top + 7);
      C.LineTo(X + 3, R.Top + 8);

      X := R.Left + 5;

      C.Moveto(X + 3, R.Top + 4);
      C.LineTo(X + 3, R.Top + 11);
      C.Moveto(X + 2, R.Top + 5);
      C.LineTo(X + 1, R.Top + 10);
      C.Moveto(X + 1, R.Top + 6);
      C.LineTo(X + 1, R.Top + 9);
      C.Moveto(X + 0, R.Top + 7);
      C.LineTo(X + 0, R.Top + 8);
...
//    ShowMessage(IntToStr(TDBGrid(Control).HelpContext));
    if((Control is TDBGrid) and Not(TDBGrid(Control).ShowHint))then
    begin
      ScV       :=TControlTest.Create(Control.Parent);
      ScV.BGColor:=XPMenu.FFSelectColor;
      ScV.Kind  := meVertical;
      ScV.Top   :=Control.Top;
      ScV.Left  :=Control.Width+Control.Left-17;
      ScV.Width :=15;
      ScV.Height:=Control.Height;
      ScV.Grid  := TCustomGrid(Control);
      ScV.Parent := Control.Parent;
      ScV.SetColors;
      TDBGrid(Control).ShowHint:=True;
    end
   else
    if(TStringGrid(Control).ScrollBars = ssBoth) then
    begin
      TStringGrid(Control).ScrollBars:=ssNone;
      Control.Height:=Control.Height-15;
      Control.Width:=Control.Width-15;

      ScH       :=TControlTest.Create(Control.Parent);
      ScH.Kind  := meHorizontal;
      ScH.Top   :=Control.Height+Control.Top;
      ScH.Left  :=Control.Left;
      ScH.Width :=Control.Width;
      ScH.Height:=15;
      ScH.Grid  := TCustomGrid(Control);
      ScH.Parent := Control.Parent;
      ScH.SetColors;

      ScV       :=TControlTest.Create(Control.Parent);
      ScV.Kind  := meVertical;
      ScV.Top   :=Control.Top;
      ScV.Left  :=Control.Width+Control.Left;
      ScV.Width :=15;
      ScV.Height:=Control.Height;
      ScV.Grid  := TCustomGrid(Control);
      ScV.Parent := Control.Parent;
