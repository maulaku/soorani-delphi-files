...
type
   ...
   public
       Function disp_date(Da : TDateTime) : string;
...
Function Tfar_date.disp_date(Da : TDateTime) : string;
var dd,n,yy,y,m,d:word;
 var   mm:real;
    ss:string;
    vv,gg:string[1];
 Begin
    ...
    if (m*100+d)<(300+dd) then
      begin
        Yy:=y-622;
        n:=DiffDay(EncodeDate(y-1,3,chk_kabise(y-1)),Da)+1;
      end
    else
     begin
       Yy:=y-621;
       n:=DiffDay(Da,EncodeDate(y,3,dd))+1;
     end;
    if n<=186 then
     begin
       mm:=int((n-1)/31)+1;
...
var Tmah :string;
    mah1:integer;
 begin
   mah1:=strtoint(copy(disp_date(Date),4,2));
  case mah1 of
   1:tmah:='ÝÑæÑÏíä';
   2:tmah:='ÇÑÏíÈåÔÊ';
   3:tmah:='ÎÑÏÇÏ';
