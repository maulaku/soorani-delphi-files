{
 ...
 Special thanks to:
   Obeliks <Obeliks@obeliks.de>

}

unit SmoothShow;

{$IFDEF VER100} { using Delphi 3.0 }
  {$DEFINE SS_D3}
{$ENDIF}
...
type
  ...
  published
    ...
    property Test: TTest read FTest write FTest stored False;
...
begin
  ...
  begin
    ...
    Shadow.Color := Color;
    OldRgn := 0;
    GetWindowRgn(Control.Handle, OldRgn);
...
begin
  ...
  begin
    ...
    if not (csDesigning in ComponentState) and Assigned(OnFinishing) then
      OnFinishing(Self);
    Shadow.Free;
    SetWindowRgn(Control.Handle, OldRgn, True);
    FBusy := False;
