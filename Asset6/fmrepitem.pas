...
uses
  ...
  ADODB, ExtCtrls, ppViewr, ppBands, ppPrnabl, ppCtrls, StdCtrls, Mask,Variants,
  Buttons, ppVar,Define, DBCtrls, RSqllookup, RSQLComboBox,RadForms,ppTypes;

type
  TrepItem = class(TRadForm)
    quItem: TADOQuery;
    dsItem: TDataSource;
    ppReport1: TppReport;
    ...
    quItemItem_Inventory: TStringField;
...
uses DabDmdle, fmItemViewer;

{$R *.DFM}

procedure TrepItem.DoPrint(var mes: TMessage);
begin
 ppViewer.Print;
end;

procedure TrepItem.DoRun(var mes: TMessage);
begin
  if(dblClass.KeyValue<>Null) AND (cbType.Text<>'') then
   begin
    ...
    quItem.SQL.Add(' ,LTrim(RTrim(Item.Item_Inventory)) Item_Inventory,LTrim(RTrim(Goods_Class.Class_Type)) Class_Type');
    ...
    if ((dblItem.Text<>'') and (dblItem.Text<>null)) then
       QuItem.SQL.Add(' And Item.Item_Code='+VartoStr(dblItem.KeyValue));
...
procedure TrepItem.quItemCalcFields(DataSet: TDataSet);
begin
...
procedure TrepItem.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  FormDeactivate(nil);
  Action:=CaFree;
end;

procedure TrepItem.FormActivate(Sender: TObject);
var mes:TMessage;
begin
  mes.Msg:=FORM_STATE;
  mes.Lparam:=47;
...
procedure TrepItem.FormDeactivate(Sender: TObject);
var mes:TMessage;
begin
 mes.Msg:=FORM_DEACT;
 Application.MainForm.Dispatch(mes);
...
procedure TrepItem.mskPreviewPageKeyPress(Sender: TObject; var Key: Char);
var
  liPage: Longint;
begin
...
procedure TrepItem.mskPreviewPercentageKeyPress(Sender: TObject;
  var Key: Char);
var
...
end; procedure TrepItem.spbPreview100PercentClick(Sender: TObject);
begin
...
procedure TrepItem.spbPreviewNextClick(Sender: TObject);
begin
  ppViewer.NextPage;
  mskPreviewPage.text:=inttostr(ppViewer.AbsolutePageNo);
end;

procedure TrepItem.spbPreviewPriorClick(Sender: TObject);
begin
  ppViewer.PriorPage;
  mskPreviewPage.text:=inttostr(ppViewer.AbsolutePageNo);
end;

procedure TrepItem.spbPreviewWholeClick(Sender: TObject);
begin
...
procedure TrepItem.spbPreviewWidthClick(Sender: TObject);
begin
...
procedure TrepItem.FormShow(Sender: TObject);
begin
  Top := 0 ;
  Left := 0 ;
end;

procedure TrepItem.cbTypeChange(Sender: TObject);
begin
 QudblClass.Close;
 if(cbType.Text <> '') then
 begin
...
procedure TrepItem.SpeedButton1Click(Sender: TObject);
begin
  inherited;
...
//  ItemVeiwer.dblClass.KeyValue:=DabData.quGoodsRecclass_code.Value;
  ...
  cbType.ItemIndex:=ItemVeiwer.quItemClass_Type.Value;
  cbTypeChange(nil);
  dblClass.KeyValue:=ItemVeiwer.quItemClass_Code.Value;
  dblClassClick(nil);

end;

procedure TrepItem.dblClassClick(Sender: TObject);
begin
  inherited;
  quGoods.Close;
  quGoods.Parameters[0].Value:=dblClass.KeyValue;
  quGoods.Open;
end;
