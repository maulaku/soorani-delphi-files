...
type
  Tfilmdet = class(TRadForm)
    ...
    Label11: TLabel;
    eddesc:TRSqlEdit;
    cbmeasure:TRSqlComboBox;
    edsize:TRSqlEdit;
    Label12: TLabel;
    cbsize:TRSqlComboBox;
...
procedure Tfilmdet.btokClick(Sender: TObject);
var bk:TBookmark;
  TempStr   : string;
  ExitWhile : Boolean;
  Str1,str2 : string;
...
begin
  ...
  1: begin
       ...
       begin
         ...
         quInsert.Parameters[1].Value:= edsub.text;
         quInsert.Parameters[2].Value:= cbtype.ItemIndex;
         quInsert.Parameters[3].Value:= edsize.text;
         quInsert.Parameters[4].Value:= cbsize.ItemIndex;
         quInsert.Parameters[5].Value:= cbmeasure.ItemIndex;
       ...
       DabData.qufilm.Open;
       DabData.qufilm.Locate('Goods_Code',TempStr,[locaseinsensitive]);

  end; //case1
  2: begin //1
       bk:= DabData.qufilm.GetBookmark;
       quUpdate.Parameters[0].Value:= edsub.text;
       quUpdate.Parameters[1].Value:=  cbtype.ItemIndex;
       quUpdate.Parameters[2].Value:= edsize.text;
       quUpdate.Parameters[3].Value:= cbsize.ItemIndex;
       quUpdate.Parameters[4].Value:= cbmeasure.ItemIndex;
       quUpdate.Parameters[5].Value:= eddesc.text;
       quUpdate.Parameters[6].Value:= DabData.qufilmgoods_code.AsString ;
       DabData.ADOC.BeginTrans;
       Try
       begin //2
         quUpdate.ExecSQL;
       ...
       DabData.qufilm.Close;
       DabData.qufilm.Open;
       DabData.qufilm .GotoBookmark(bk);
       DabData.qufilm .FreeBookmark(bk);
...
procedure Tfilmdet.btcaClick(Sender: TObject);
begin
   filmdet.Close;
end;


procedure Tfilmdet.Button1Click(Sender: TObject);
begin
...
procedure Tfilmdet.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree ;
end;
procedure Tfilmdet.btGoodUpClick(Sender: TObject);
begin
   ShowMal := TShowMal.create(Application);
   SetFormColor(ShowMal);
   ShowMal.ShowModal ;
...
procedure Tfilmdet.FormActivate(Sender: TObject);
begin
   if(mde=1) then
   begin
