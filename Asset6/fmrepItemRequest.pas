...
type
  TrepItemRequest = class(TRadForm)
    ppViewer: TppViewer;
    ppViewer1: TppViewer;
    Panel2: TPanel;
    ...
    quItemReqEmp_No: TStringField;
    quItemReqFullName: TStringField;
...
var
  repItemRequest: TrepItemRequest;
...
procedure TrepItemRequest.mskPreviewPageKeyPress(Sender: TObject; var Key: Char);
var
  liPage: Longint;
begin
...
procedure TrepItemRequest.mskPreviewPercentageKeyPress(Sender: TObject;
  var Key: Char);
var
...
end; procedure TrepItemRequest.spbPreview100PercentClick(Sender: TObject);
begin
...
procedure TrepItemRequest.spbPreviewNextClick(Sender: TObject);
begin
  ppViewer.NextPage;
  mskPreviewPage.text:=inttostr(ppViewer.AbsolutePageNo);
end;

procedure TrepItemRequest.spbPreviewPriorClick(Sender: TObject);
begin
  ppViewer.PriorPage;
  mskPreviewPage.text:=inttostr(ppViewer.AbsolutePageNo);
end;

procedure TrepItemRequest.spbPreviewWholeClick(Sender: TObject);
begin
...
procedure TrepItemRequest.spbPreviewWidthClick(Sender: TObject);
begin
...
procedure TrepItemRequest.quItemReqCalcFields(DataSet: TDataSet);
begin
...
procedure TrepItemRequest.DoPrint(var mes: TMessage);
begin
 ppViewer.Print;
end;

procedure TrepItemRequest.DoRun(var mes: TMessage);
...
begin
   ...
   quItemReq.SQL.add(' ,EMP.Emp_No,(Emp.Name+'' ''+Emp.Family) as FullName');
   ...
   quItemReq.SQL.Add(' left outer join Emp on Emp.Emp_No=Item_Request.Creator');
   quItemReq.sql.add(s);
   quItemReq.Open;
   ppViewer.reset;
...
procedure TrepItemRequest.cbTypeChange(Sender: TObject);
begin
 if(cbType.Text<>'') then
  begin
   qudblClass.Close;
...
procedure TrepItemRequest.dblClassClick(Sender: TObject);
begin
...
procedure TrepItemRequest.FormShow(Sender: TObject);
begin
  quDblOrg.Close;
  quDblOrg.Open;
end;

procedure TrepItemRequest.FormActivate(Sender: TObject);
var mes:TMessage;
begin
  mes.Msg:=FORM_STATE;
  mes.Lparam:=61;
...
procedure TrepItemRequest.FormDeactivate(Sender: TObject);
var mes:TMessage;
begin
 mes.Msg:=FORM_DEACT;
 Application.MainForm.Dispatch(mes);
...
procedure TrepItemRequest.FormClose(Sender: TObject; var Action: TCloseAction);
begin
 FormDeactivate(nil);
  Action:=CaFree;
end;

procedure TrepItemRequest.ppVariable1Calc(Sender: TObject; var Value: Variant);

begin
  Value:=Value+1;
end;

procedure TrepItemRequest.ppVariable1Reset(Sender: TObject;
  var Value: Variant);
begin
 Value:=1;
end;

procedure TrepItemRequest.dblOrgClick(Sender: TObject);
begin
